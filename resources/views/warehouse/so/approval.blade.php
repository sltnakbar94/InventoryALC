@if (@$crud->entry->status == 0 || @$crud->entry->status == 5)
    @if (!empty(@$crud->entry->details->first()) && !empty(@$crud->entry->pic_customer) && !empty(@$crud->entry->pic_supplier) && backpack_user()->hasRole('sales'))
    ...
    <div class="card-footer" align="center">
        <div class="dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <a class="dropdown-item" href="{{backpack_url('salesorder/'.$crud->entry->id.'/accept')}}">Setujui</a>
                <a class="dropdown-item" data-target="#revisionModalSalesOrder" data-toggle="modal">Revisi</a>
                <a class="dropdown-item" href="{{backpack_url('salesorder/'.$crud->entry->id.'/denied')}}">Tolak</a>
            </div>
        </div>
    </div>
@endif
